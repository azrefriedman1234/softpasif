...
class MainActivity : AppCompatActivity() {
    ...
    // MAX_MESSAGES_CAP_100
    private val MAX_MESSAGES = 100
    private val NEWEST_FIRST = false

    private fun trimMessagesToMax() {
        try {
            while (messages.size > MAX_MESSAGES) {
                val removed = if (NEWEST_FIRST) messages.removeAt(messages.size - 1) else messages.removeAt(0)
                cleanupMessageFiles(removed)
            }
            // simplest + safe
    ...
    private fun cleanupMessageFiles(msg: org.drinkless.tdlib.TdApi.Message) {
        // Delete only files that belong to this app (cacheDir/filesDir). Includes tdlib folder under filesDir.
        ...
        try {
            when (val c = msg.content) {
                is org.drinkless.tdlib.TdApi.MessagePhoto -> {
                    ...
                    for (sz in sizes) addPath(sz.photo?.local?.path)
                    addPath(c.photo.minithumbnail?.toString())
                }
                is org.drinkless.tdlib.TdApi.MessageVideo -> {
                    addPath(c.video.video?.local?.path)
                    addPath(c.video.thumbnail?.file?.local?.path)
                    addPath(c.video.minithumbnail?.toString())
                }
                is org.drinkless.tdlib.TdApi.MessageAnimation -> {
                    addPath(c.animation.animation?.local?.path)
                    addPath(c.animation.thumbnail?.file?.local?.path)
                    addPath(c.animation.minithumbnail?.toString())
                }
                is org.drinkless.tdlib.TdApi.MessageDocument -> {
                    addPath(c.document.document?.local?.path)
                    addPath(c.document.thumbnail?.file?.local?.path)
                    addPath(c.document.minithumbnail?.toString())
                }
                else -> {}
            }
        } catch (_: Exception) {}
        ...
        for (pp in paths.distinct()) {
            try {
                val f = java.io.File(pp)
                if (!f.exists()) continue
                val canon = f.canonicalFile
                val inFiles = canon.path.startsWith(filesRoot.path)
                val inCache = canon.path.startsWith(cacheRoot.path)
                if (inFiles || inCache) {
                    runCatching { canon.delete() }
                }
            } catch (_: Exception) {}
        }
    }
